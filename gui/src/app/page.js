...
export default function Home() {
    ...
    const handleDownloadSourcebook = async (slug, title, ruleset) => {
        ...
        if (outputPath) {
            ...
            setLogs([`Starting download for ${title}...`]);
            ...
            if (result.success) {
                ...
                setLogs((prev) => [...prev, `\nDownload failed. Check the logs above.`]);
            }
            setTimeout(() => setIsDownloading(false), 3000);
        }
    };

    const handleDownloadSelectedRules = async () => {
        ...
        setIsDownloading(true);
        setLogs([`Starting bulk rule download to ${outputPath}...`]);

        let allSuccess = true;
        for (const category of selectedRules) {
            setLogs((prev) => [...prev, `\n--- Downloading ${category} ---`]);
            const result = await window.electronAPI.startExtractionRules(
                category,
                includeHomebrew,
                outputPath,
            );
            ...
            if (category === 'classes') {
                setLogs((prev) => [...prev, `\n--- Downloading subclasses ---`]);
                const subResult = await window.electronAPI.startExtractionRules(
                    'subclasses',
                    includeHomebrew,
                    outputPath,
                );
        ...
        if (allSuccess) {
            setLogs((prev) => [...prev, `\nAll selected rules downloaded successfully!`]);
        } else {
    ...
    const accessibleSources = useMemo(() => {
        ...
        return sources.filter((source) => {
            if (source.title.startsWith('D&D Beyond Drops') || source.slug === 'ddb-drops')
                return false;
    ...
    const publishers = useMemo(() => {
        let pubs = Array.from(
            new Set(accessibleSources.map((s) => s.publisher).filter(Boolean)),
        ).sort();
    ...
    const filteredSources = useMemo(() => {
        return accessibleSources
            .filter((source) => {
                ...
                if (rulesetFilter !== 'All' && (source.ruleset || '5e') !== rulesetFilter)
                    return false;
    ...
    return (
        <main className="pt-8 px-8 pb-0 h-full flex flex-col">
            <header className="flex justify-between items-center mb-8 shrink-0">
                <div>
                    <h1 className="text-5xl mb-2 tracking-wide flex items-baseline">
                        ...
                        <span
                            className="text-white font-bold text-[1.12em]"
                            style={{
                ...
                <div className="relative flex flex-col items-end">
                    ...
                    {!isSignedIn && authMsg && (
                        <span className="absolute top-full right-0 text-sm mt-2 text-green-400 whitespace-nowrap">
                            {authMsg}
                        </span>
                    )}
                    {isSignedIn && (
                ...
                <div className="flex flex-col h-full overflow-hidden">
                    ...
                    <div className="flex items-end space-x-4 mb-6 shrink-0 p-2 w-full overflow-x-auto styled-scrollbar">
                        ...
                        <div className="flex flex-col gap-2 shrink-0">
                            <span className="text-gray-400 font-[800] text-[10px] tracking-wider uppercase">
                                Rule Set:
                            </span>
                            <div className="flex bg-gray-900 rounded-lg p-1 border border-gray-800 items-center">
                                <Chiclet
                                    label="All"
                                    isActive={rulesetFilter === 'All'}
                                    onClick={() => setRulesetFilter('All')}
                                />
                                {rulesets.map((rs) => (
                                    <Chiclet
                                        key={rs}
                                        label={rs}
                                        isActive={rulesetFilter === rs}
                        ...
                        <div className="flex flex-col gap-2 shrink-0">
                            ...
                            <div className="flex bg-gray-900 rounded-lg p-1 border border-gray-800 items-center">
                                ...
                                <Chiclet
                                    label="Sourcebooks"
                                    isActive={categoryFilter === 'Sourcebooks'}
                                    onClick={() => setCategoryFilter('Sourcebooks')}
                                />
                                <Chiclet
                                    label="Adventures"
                                    isActive={categoryFilter === 'Adventures'}
                                    onClick={() => setCategoryFilter('Adventures')}
                                />
                            </div>
                        </div>
                        ...
                        <div className="flex flex-col gap-2 shrink-0">
                            ...
                            <span className="text-transparent font-[800] text-[10px] tracking-wider uppercase invisible">
                                Search:
                            </span>
                    ...
                    <div className="overflow-y-auto flex-1 pr-2 styled-scrollbar">
                        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 2xl:grid-cols-7 gap-4">
                            {filteredSources.map((source) => {
                                ...
                                const isAccessible =
                                    source.isOwned || source.isFree || source.isSharedWithMe;
                                const lastDownloaded = history[slug];
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        <div className="relative p-3 flex-1 flex flex-col bg-gradient-to-t from-gray-900 to-gray-800">
                                            ...
                                            <h3
                                                className="font-sans font-[800] text-sm md:text-base text-gray-100 mb-1 line-clamp-2 leading-tight"
                                                title={source.title}
                                            >
                                                {source.title}
                                            </h3>
                                            <p
                                                className="text-gray-400 text-[10px] md:text-xs font-sans mb-3 capitalize line-clamp-1"
                                            ...
                                            <div className="mt-auto flex flex-col space-y-2">
                                                ...
                                                {isAccessible ? (
                                                    <button
                                                        onClick={() =>
                                                            handleDownloadSourcebook(
                                                                slug,
                                                                source.title,
                                                                source.ruleset || '5e',
                                                            )
                                                        }
                                                        className={`w-full py-2 flex items-center justify-center ${lastDownloaded ? 'bg-green-900/40 hover:bg-green-800 text-green-300 border border-green-700/50' : 'bg-gray-700 hover:bg-[#E2E2E2] hover:text-gray-900 text-gray-100'} rounded transition-colors duration-300 font-medium text-sm`}
                                                    >
                                                        Download <MarkdownSvg />
                                                    </button>
                                                ) : (
                                                    <a
                ...
                <div className="flex-1 flex flex-col pt-2">
                    <div className="flex justify-between items-center mb-6">
                        ...
                        <div className="flex items-center space-x-4">
                            ...
                            >
                                {selectedRules.size === ruleCategories.length
                                    ? 'Deselect All'
                                    : 'Select All'}
                            </button>
                            <button
                                onClick={handleDownloadSelectedRules}
                            ...
                            >
                                Download Selected ({selectedRules.size}) <MarkdownSvg />
                            </button>
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-[repeat(auto-fill,minmax(200px,1fr))] gap-4 overflow-y-auto pr-2 styled-scrollbar p-1">
                        {ruleCategories.map((cat) => {
                            const lastDownloaded = history[cat.slug];
                            return (
                                        ...
                                        <div className="flex items-center space-x-3">
                                            {ruleIcons[cat.slug] && (
                                                <img
                                                    ...
                                                    className={`h-[1.25em] w-auto object-contain transition-opacity duration-300 ${selectedRules.has(cat.slug) || lastDownloaded ? 'opacity-100' : 'opacity-60'}`}
                                            ...
                                            <div className="flex flex-col items-start justify-center">
                                                <h3
                                                    className={`font-sans font-[800] text-[1.1rem] leading-none ${!selectedRules.has(cat.slug) && lastDownloaded ? 'text-green-300' : 'text-gray-200'}`}
                                                >
                                                    {cat.label}
                                                </h3>
                                            </div>
                                    ...
                                    {lastDownloaded && (
                                        <div className="mt-1 flex items-center gap-1.5">
                                            <span className="text-[10px] text-green-400 font-mono">
                                                ✓ Downloaded:{' '}
                                                {new Date(lastDownloaded).toLocaleDateString()}
                                            </span>
                                        </div>
                                    )}
                                </div>
            ...
            {activeTab === 'tools' && <ToolsTab onClearHistory={() => setHistory({})} />}
                ...
                <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-50 p-8">
                    <div className="bg-gray-900 border border-[#E2E2E2]/30 rounded-xl shadow-[0_0_50px_rgba(226,226,226,0.1)] w-full max-w-4xl h-[70vh] flex flex-col overflow-hidden">
                        <div className="bg-black/50 px-6 py-4 border-b border-gray-800 flex justify-between items-center">
                            <h2 className="font-sans font-[800] tracking-wider text-xl text-gray-200">
                                Extraction Progress
                            </h2>
                        </div>
            ...
            {/* Mockup Legal Disclaimer Modal */}
            {showDisclaimer && (
                <div className="fixed inset-0 bg-black/60 backdrop-blur-md flex items-center justify-center z-[100] p-4 sm:p-8">
                    <div className="bg-white w-full max-w-4xl flex flex-col max-h-[90vh] shadow-2xl rounded-sm overflow-hidden">
                        {/* Header */}
                        <div className="text-center pt-8 px-6 sm:px-12 pb-6 shrink-0 border-b border-gray-100 bg-white z-10">
                            <h1 className="text-4xl sm:text-5xl font-bold tracking-tight mb-2 flex items-center justify-center">
                                <span className="text-red-600 font-normal" style={{ fontFamily: 'var(--font-marcellus)' }}>BEYOND</span>
                                <span className="text-black" style={{ fontFamily: "'Gill Sans', 'Gill Sans MT', Calibri, sans-serif", fontSize: "calc(100% + 1pt)", transform: "translateY(-1px)" }}>MARKDOWN</span>
                            </h1>
                            <h2 className="text-xl sm:text-2xl font-sans font-bold text-black">
                                Disclaimer
                            </h2>
                        </div>
                        
                        {/* Content */}
                        <div className="text-black font-sans text-sm leading-relaxed space-y-5 flex-grow overflow-y-auto px-6 sm:px-12 py-6">
                            <p>
                                This application, Beyond Markdown, is unofficial Fan Content permitted under the <a href="https://company.wizards.com/en/legal/fancontentpolicy" target="_blank" rel="noreferrer" className="text-blue-600 hover:underline">Fan Content Policy</a>. Not approved/endorsed by Wizards. Portions of the materials used are property of Wizards of the Coast. ©Wizards of the Coast LLC.
                            </p>
                            
                            <p>
                                This application is designed solely for <strong>format shifting</strong> - allowing the authorized owner of the digital content to reformat content they have an active license to access into Markdown. The intended use for personal accessibility, reference, use with private, non-commercial utilities, including, but not limited to Virtual Table Top (VTT) applications and locally hosted AI/LLM instances.
                            </p>
                            
                            <p>
                                This application contains <strong>zero</strong> proprietary game data, text, or image assets. Users must provide their own authenticated session and can only extract content they have legitimately licensed. This script does not bypass paywalls, DRM, or access controls.
                            </p>
                            
                            <p>
                                You, the user of this application, are expressly forbidden from distributing or monetizing the Markdown files generated by this application. See <a href="https://www.dndbeyond.com/en/terms-conditions" target="_blank" rel="noreferrer" className="text-blue-600 hover:underline">D&D Beyond Terms & Conditions</a> for further information on what licensed access to their content does or does not permit. By using this application, you accept the risks and repercussions of any potential violation(s) of the D&D Beyond Terms & Conditions, take full responsibility for the data you extract, and agree not to distribute copyrighted material.
                            </p>
                            ...
                            <p>
                                By clicking “I Understand & Accept”, you agree to take full responsibility for adherence to the aforementioned policies, terms, and conditions and compliance with any applicable laws regarding copyrighted material.
                            </p>
                            ...
                            <div className="pt-2 flex items-center gap-3">
                                <input 
                                    type="checkbox" 
                                    id="acceptDisclaimer" 
                                    className="w-4 h-4 border-2 border-black rounded-sm cursor-pointer accent-black"
                                    checked={disclaimerChecked}
                                    onChange={(e) => setDisclaimerChecked(e.target.checked)}
                                />
                                <label htmlFor="acceptDisclaimer" className="text-black cursor-pointer select-none font-medium">
                                    I Understand & Accept
                                </label>
                            </div>
                        </div>
                        ...
                        {/* Footer / Button */}
                        <div className="flex justify-end shrink-0 px-6 sm:px-12 pb-8 pt-4 border-t border-gray-100 bg-white z-10">
                            <button
                                onClick={() => {
                                ...
                                className={`px-10 py-2.5 rounded-sm font-bold tracking-wider transition-all ${
                                    disclaimerChecked 
                                        ? 'bg-[#A8D08D] hover:bg-[#92C471] text-black cursor-pointer shadow-sm' 
                                        : 'bg-[#E2EFE1] text-gray-400 cursor-not-allowed'
                                }`}
                            >
                                DONE
                            </button>
