...
export default function MarkdownViewer({ content, filePath, onClose, onNavigate, targetAnchor }) {
    ...
    // Automatically scroll to target anchor when content or anchor changes
    useEffect(() => {
        if (targetAnchor) {
            // Slight delay to allow ReactMarkdown to finish rendering the DOM
            setTimeout(() => {
    ...
    const components = {
        ...
        a: ({ node, href, children, ...props }) => {
            ...
            return (
                <a
                    href={href}
                    className="text-red-400 hover:text-red-300 underline decoration-red-400/30 hover:decoration-red-400 transition-colors cursor-pointer"
                    onClick={(e) => {
                        e.preventDefault();
        ...
        blockquote: ({ children, ...props }) => (
            <blockquote
                className="border-l-4 border-gray-600 pl-4 py-1 mb-4 text-gray-400 bg-gray-800/30 rounded-r"
                {...props}
            >
                {children}
            </blockquote>
        ...
        code: ({ node, inline, className, children, ...props }) => {
            return inline ? (
                <code
                    className="bg-gray-800 text-red-200 px-1.5 py-0.5 rounded text-sm font-mono"
                    {...props}
                >
                    {children}
                </code>
        ...
        th: ({ children, ...props }) => (
            <th className="bg-gray-800 px-4 py-2 font-semibold border-b border-gray-700" {...props}>
                {children}
            </th>
        ),
        td: ({ children, ...props }) => (
            <td className="px-4 py-2 border-b border-gray-700/50 bg-gray-900/50" {...props}>
                {children}
            </td>
        ),
