...
export default function RulesetConverter({ filePath }) {
  ...
  const handleConvert = async () => {
    ...
    if (outputPath) {
      ...
      setLogs([`Starting conversion to ${targetRuleset}...`, `Source: ${filePath}`, `Destination: ${outputPath}`]);
      
      const result = await window.electronAPI.convertLocalFile(filePath, targetRuleset, outputPath);
      if (result.success) {
    ...
    <div className="flex-1 flex flex-col items-center justify-start pt-12">
      <div className="bg-gray-900 border border-gray-700 rounded-xl p-8 max-w-lg w-full shadow-lg">
        ...
        <div className="mb-6 space-y-4">
          <div>
            <label className="block text-[10px] uppercase tracking-wider text-gray-500 font-[800] mb-1">Source File</label>
            <div className="font-mono text-sm text-gray-300 truncate bg-gray-950 p-2 rounded border border-gray-800" title={filePath}>
              {filePath}
            </div>
          </div>
          ...
          <div className="flex gap-4 items-center">
            <div className="flex-1">
              <label className="block text-[10px] uppercase tracking-wider text-gray-500 font-[800] mb-1">Detected Ruleset</label>
            ...
            <svg className="w-6 h-6 text-gray-500 mt-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M14 5l7 7m0 0l-7 7m7-7H3" />
            </svg>

            <div className="flex-1">
              <label className="block text-[10px] uppercase tracking-wider text-gray-500 font-[800] mb-1">Target Ruleset</label>
              <select 
                value={targetRuleset}
      ...
      {isExtracting && (
        <div className="mt-8 bg-black/50 border border-[#E2E2E2]/20 rounded-xl w-full max-w-2xl h-64 flex flex-col overflow-hidden shadow-inner">
          <div className="bg-gray-900/80 px-4 py-2 border-b border-gray-800 flex justify-between items-center shrink-0">
            <h3 className="font-sans font-[800] text-sm text-gray-400">Conversion Logs</h3>
          </div>
