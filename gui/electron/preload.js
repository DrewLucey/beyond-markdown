...
contextBridge.exposeInMainWorld('electronAPI', {
    ...
    getSchemaContent: () => ipcRenderer.invoke('get-schema-content'),
    ...
    startExtractionSourcebook: (slug, outputPath) =>
        ipcRenderer.invoke('start-extraction-sourcebook', slug, outputPath),
    startExtractionRules: (category, includeHomebrew, outputPath) =>
        ipcRenderer.invoke('start-extraction-rules', category, includeHomebrew, outputPath),
    ...
    convertLocalFile: (filePath, targetRuleset, outputPath) =>
        ipcRenderer.invoke('convert-local-file', filePath, targetRuleset, outputPath),
    openInBrowser: (markdown, title) => ipcRenderer.invoke('open-in-browser', markdown, title),
    onExtractionLog: (callback) =>
        ipcRenderer.on('extraction-log', (event, data) => callback(data)),
