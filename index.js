    ...
    const { action } = await inquirer.prompt([
        {
            ...
            choices: [
                ...
                {
                    name: '4. Convert Sourcebook/Adventure - 5e (2014) <-> 5.5e (2024)',
                    value: 'convert',
                },
...
async function handleExtractSingle() {
    ...
    if (choices.length === 0) {
        console.log(
            "\nNo owned sourcebooks found. Please run 'node src/scripts/library.js' first to map your library.\n",
        );
        return;
    }
    ...
    await runScript(path.join(__dirname, 'src/scripts/extract.js'), [slug]);
    await runScript(path.join(__dirname, 'src/scripts/stitcher.js'), [slug]);
}
...
async function handleExtractAll() {
    ...
    for (const choice of choices) {
        ...
        try {
            await runScript(path.join(__dirname, 'src/scripts/extract.js'), [choice.value]);
            await runScript(path.join(__dirname, 'src/scripts/stitcher.js'), [choice.value]);
        } catch (e) {
...
async function handleExtractRules() {
    ...
    await runScript(path.join(__dirname, 'src/scripts/bulk_extract.js'), [category]);
    await runScript(path.join(__dirname, 'src/scripts/repo_stitcher.js'), [category]);
}
    ...
    const { target, ruleset } = await inquirer.prompt([
        {
            type: 'input',
            name: 'target',
            message: 'Enter the category or book slug to convert (e.g., species, cos):',
        },
        { type: 'select', name: 'ruleset', message: 'Target Ruleset:', choices: ['2024', '2014'] },
    ]);

    if (!target) return;
    await runScript(path.join(__dirname, 'src/scripts/rules_converter.js'), [
        target.trim(),
        ruleset,
    ]);
}
...
async function handleCharacter() {
    await runScript(path.join(__dirname, 'src/scripts/character_compiler.js'));
}
...
async function init() {
    ...
    try {
        await runScript(path.join(__dirname, 'src/scripts/library.js'));
    } catch (err) {
