...
const defaultOutputFile = path.resolve(__dirname, `../sources/${rulesetFolder}/${finalFileName}`);
const outputFile = process.argv[3] ? path.resolve(process.argv[3]) : defaultOutputFile;
// ---------------------------------------------------------
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (headingMatch) {
            ...
            if (linkMatch) {
                ...
                rawText = rawText.replace(/[*_~\x60()]/g, '');
        ...
        if (url.includes('/sources/') || url.includes('/compendium/')) {
            ...
            if (!validChapterIds.has(targetSlug)) {
                ...
                for (const [mapSlug, meta] of Object.entries(rulesMap)) {
                    const legacyPathName = meta.name ? `/sources/${meta.name.toLowerCase()}` : null;
                ...
                if (crossBookSlug && crossBookSlug.toLowerCase() !== bookSlug.toLowerCase()) {
