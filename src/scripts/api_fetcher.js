    ...
    try {
        ...
        if (TARGET_TYPE === 'monsters') {
            try {
                ...
                const configRes = await axios.get('https://www.dndbeyond.com/api/config/json', {
                    headers: reqHeaders,
                });
        ...
        if (TARGET_TYPE === 'monsters') {
            ...
            while (hasMore) {
                ...
                const res = await axios.get(`${ENDPOINTS.monsters}?skip=${skip}&take=${take}`, {
                    headers: reqHeaders,
                });
        ...
        // ROUTE 2: Spell Class-Sweep
            ...
            for (const classId of classIds) {
                try {
                    const res = await axios.get(`${ENDPOINTS.subclasses}?baseClassId=${classId}`, {
                        headers: reqHeaders,
                    });
                    if (res.data && res.data.data) {
        ...
        for (let rawItem of items) {
            ...
            let metaData = '',
                desc = item.description || item.snippet || item.characteristicsDescription || '';
            ...
            if (TARGET_TYPE === 'monsters') {
                ...
                const stats = (item.stats || [])
                    .map((s) => {
                        const key =
                            (ddbConfig.stats || []).find((st) => st.id === s.statId)?.key || 'STAT';
                        return `**${key}:** ${s.value}`;
                    })
                    .join(' | ');
