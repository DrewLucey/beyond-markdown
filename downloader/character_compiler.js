  ...
  try {
    ...
    if (!input) {
      ...
      const selection = await askQuestion(
        `Select a character (0-${characters.length}): `,
      );
    ...
    console.log(
      `Successfully fetched character: ${char.name}. Compiling sheet...`,
    );
    ...
    const background =
      char.background?.definition?.name || "Unknown Background";
    ...
    allModifiers.forEach((mod) => {
      if (mod.type === "bonus" && mod.subType.includes("-score")) {
        ...
        const statId = Object.keys(STAT_MAP).find(
          (k) => STAT_MAP[k].key === statKey,
        );
        if (statId && mod.value) {
    ...
    if (err.response && err.response.status === 403) {
      console.error(
        "403 Forbidden: Ensure your CobaltSession token is valid and active.",
      );
    }
  }
}
