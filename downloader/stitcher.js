...
// Lifted rulesMap so it can be accessed by the link transformer
let rulesMap = {};
...
if (fs.existsSync(mapFilePath)) {
  try {
    ...
    console.warn(
      "Could not read ruleset_map.json. Proceeding with default tags.",
    );
  }
} else {
  console.warn(
    "ruleset_map.json not found. Run `node downloader/library.js` for better AI metadata and naming.",
  );
}
...
const sourceDir = path.resolve(
  __dirname,
  `../sources/${rulesetFolder}/atomic/${TARGET_BOOK}`,
);

const finalFileName = `${bookTitle} (${bookMeta.ruleset}).md`;
const outputFile = path.resolve(
  __dirname,
  `../sources/${rulesetFolder}/${finalFileName}`,
);
// ---------------------------------------------------------
...
function buildStitcherManifest(dirPath) {
  ...
  if (!fs.existsSync(dirPath)) {
    throw new Error(
      `Directory not found. Run extract.js on ${TARGET_BOOK} first.`,
    );
  }
  ...
  if (!fs.existsSync(indexFilePath)) {
    throw new Error(
      `Critical Error: index.md not found in ${dirPath}. The stitcher relies on the index to determine reading order.`,
    );
  }

  const availableFiles = fs
    .readdirSync(dirPath)
    .filter((f) => f.endsWith(".md") && !f.startsWith("_master__"));
  console.log(
    `Found ${availableFiles.length} atomic files. Mapping reading order from index.md...`,
  );
  ...
  $("a").each((_, el) => {
    ...
    const matchedFileName = availableFiles.find(
      (f) => f.replace(".md", "").toLowerCase() === targetSlug.toLowerCase(),
    );
...
function transformMarkdownLinks(
  ...
  return content.replace(
    ...
    (match, text, rawUrlGroup) => {
      ...
      if (url.startsWith("#")) {
        // FIX: Explicitly ignore URN internal links so we don't corrupt their tooltips
      ...
      if (url.includes("/sources/") || url.includes("/compendium/")) {
        ...
        cleanUrl =
          hashIndex !== -1 ? cleanUrl.substring(0, hashIndex) : cleanUrl;
        ...
        // --- CROSS-BOOK LINK DETECTION ---
        // If the target slug isn't a chapter in THIS book, cross-reference the library map
        if (!validChapterIds.has(targetSlug)) {
          const relativeUrlPath = cleanUrl.replace(/^https?:\/\/[^\/]+/, "");

          let crossBookSlug = null;
          for (const [mapSlug, meta] of Object.entries(rulesMap)) {
            const legacyPathName = meta.name
              ? `/sources/${meta.name.toLowerCase()}`
              : null;
            const legacyPathSlug = `/sources/${mapSlug.toLowerCase()}`;

            // Check if the URL matches the modern path, the legacy acronym, or the strict slug
            if (
              (meta.path && relativeUrlPath.startsWith(meta.path)) ||
              (legacyPathName && relativeUrlPath.startsWith(legacyPathName)) ||
              relativeUrlPath.startsWith(legacyPathSlug)
            ) {
              crossBookSlug = mapSlug;
              break;
            }
          }

          // If a match is found to a different book, override the link to target the root RAG anchor
          if (
            crossBookSlug &&
            crossBookSlug.toLowerCase() !== bookSlug.toLowerCase()
          ) {
      ...
      if (targetSlug && validChapterIds.has(targetSlug)) {
        ...
        const originalUrl = trueHash
          ? `${baseUrl}${urlTargetSlug}#${trueHash}`
          : `${baseUrl}${urlTargetSlug}`;
  ...
  try {
    ...
    for (let i = 0; i < manifest.length; i++) {
      ...
      if (chapterSlug.toLowerCase() === "index") {
        chapterText = applyHierarchicalHeadingIds(
          chapterText,
          TARGET_BOOK,
          chapterSlug,
        );
      ...
      chapterText = transformMarkdownLinks(
        chapterText,
        manifest,
        chapterSlug,
        TARGET_BOOK,
