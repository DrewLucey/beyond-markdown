...
const LIBRARY_URL =
  "https://www.dndbeyond.com/en/library?ownership=owned-shared";
...
async function crawlLibrary() {
  try {
      ...
      if (unescapedChunk.includes('{"sources":[')) {
        ...
        if (endIdx !== -1) {
          try {
            ...
            console.error(
              "JSON Parse failed on extracted stream array:",
              e.message,
            );
          }
        }
    ...
    if (sourceCount === 0) {
      ...
      console.log(
        `✅ Metadata Dictionary successfully saved to: ${OUTPUT_MAP_FILE}`,
      );
      console.log(`Mapped ${sourceCount} sourcebooks.`);

      // Quick visual verification for the user
      const sample55e = Object.values(sourceMap).find(
        (s) => s.ruleset === "5.5e",
      );
