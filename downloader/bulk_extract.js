...
// Explicit URL-to-Singular mapping table
const CATEGORY_SINGULAR_MAP = {
    '/classes': 'class',
    '/backgrounds': 'background',
    '/species': 'species',
    '/races': 'species', // Supplant 'races' entirely with 'species'
    '/feats': 'feat',
    '/spells': 'spell',
    '/equipment': 'item',
...
async function runBulkPipeline() {
    ...
    let categoryFolderName = targetPath.replace(/[^a-zA-Z0-9_-]/g, '');
    if (categoryFolderName === 'races') categoryFolderName = 'species'; // Supplant directory name

    const outputDir = path.join(__dirname, '..', 'sources', 'repositories', categoryFolderName);

    if (!fs.existsSync(outputDir)) {
        fs.mkdirSync(outputDir, { recursive: true });
    }

    console.log(`\n--- Starting Bulk Extract for ${categoryFolderName.toUpperCase()} ---`);
    ...
    for (let i = 0; i < items.length; i++) {
        ...
        try {
            ...
            if ($content && $content.length > 0) {
                ...
                const sourceEl = $content.find('.source, .spell-source, .monster-source, .equipment-source, .magic-item-source, .source-summary, .static-container-footer-content span').first();
                ...
                // --- SINGULAR RESOLUTION ---
                const singularCategory = CATEGORY_SINGULAR_MAP[targetPath] || categoryFolderName.replace(/es$/, '').replace(/s$/, '');
                ...
                successCount++;
                process.stdout.write(`\r[${i+1}/${items.length}] Extracted: ${item.name.substring(0, 40).padEnd(40)}`);
            }
        } catch (e) { 
