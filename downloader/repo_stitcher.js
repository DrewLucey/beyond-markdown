...
const formattedTitle = TARGET_REPO.split(/[-_]/).map(w => w.charAt(0).toUpperCase() + w.slice(1)).join(' ');
...
async function runRepoStitcher() {
    try {
        ...
        for (const ruleset in rulesetGroups) {
            ...
            console.log(`Stitching ${rulesetGroups[ruleset].length} items for ruleset [${ruleset}]...`);
