...
async function runRepoStitcher() {
  try {
    console.log(
      `--- Starting Repository Stitcher for: ${formattedTitle.toUpperCase()} ---`,
    );
    ...
    console.log(
      `Found ${files.length} items to stitch. Analyzing ruleset metadata...\n`,
    );
    ...
    for (const ruleset in rulesetGroups) {
      ...
      const outputFile = path.resolve(
        __dirname,
        `../sources/repositories/${finalFileName}`,
      );

      if (fs.existsSync(outputFile)) {
        console.log(
          `⚠️ WARNING: Master file '${finalFileName}' already exists.`,
        );
      ...
      console.log(
        `✔ Success! Saved to: ${finalFileName} (${fileSizeInMegabytes} MB)\n`,
      );
    }
  } catch (err) {
