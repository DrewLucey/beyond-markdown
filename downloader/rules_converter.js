...
if (!CATEGORY || !RAW_TARGET) {
  console.error(
    "Usage: node rules_converter.js <category_or_slug> <target_ruleset>",
  );
...
if (normalizedTarget === "5e" || normalizedTarget === "2014") {
  ...
  console.error(
    "Invalid target ruleset. Please use '5e', '2014', '5.5e', or '2024'.",
  );
  process.exit(1);
}
  ...
  try {
    ...
    if (mapEntry && mapEntry.ruleset) {
      ...
      let normalizedMapRuleset =
        mapRuleset.includes("5.5") || mapRuleset.includes("2024")
          ? "2024"
          : "2014";
  ...
  categories.forEach((cat) => {
    ...
    files.forEach((file) => {
      ...
      if (nameMatch && rulesetMatch && urnMatch) {
        ...
        ruleset =
          ruleset.includes("5.5") || ruleset.includes("2024") ? "2024" : "2014";
...
function convertWording(text, direction) {
  let result = text;
  const urnMap =
    direction === "2014to2024" ? URN_MAP_2014_TO_2024 : URN_MAP_2024_TO_2014;
    ...
    result = result.replace(
      ...
      (match, linkText, fullUrl, hash, suffix) => {
        ...
        if (GLOSSARY_LINKS_2014_TO_2024[hashLower]) {
          ...
          if (
            linkText.toLowerCase().includes(oldTextClean) &&
            oldTextClean.length > 2
          ) {
...
function processConversion() {
  ...
  if (mode === "repo") {
    ...
    const files = fs
      ...
      .filter((f) => f.endsWith(".md") && !f.startsWith("_"));
    console.log(
      `Mode: REPOSITORY. Scanning ${files.length} files in ${repoDir}...`,
    );
    ...
    if (!mapEntry) {
      console.error(
        `Could not find "${CATEGORY}" in ruleset_map.json. Cannot convert sourcebook.`,
      );
      process.exit(1);
    }
    ...
    if (!sourceFile) {
      ...
      console.error(
        `Ensure you have run the stitcher on this sourcebook first.`,
      );
      process.exit(1);
    }
    ...
    console.log(
      `Mode: SOURCEBOOK. Processing stitched macro file: ${sourceFile}`,
    );
    ...
    const sourcebookTagRegex = new RegExp(
      `(<SOURCEBOOK[^>]*id=")${safeCategory}(")`,
      "i",
    );
