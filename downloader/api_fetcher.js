...
const ENDPOINTS = {
  classes:
    "https://character-service.dndbeyond.com/character/v5/game-data/classes?includeCustomItems=true",
  subclasses:
    "https://character-service.dndbeyond.com/character/v5/game-data/subclasses",
  spells:
    "https://character-service.dndbeyond.com/character/v5/game-data/spells",
  items:
  ...
  backgrounds:
    "https://character-service.dndbeyond.com/character/v5/game-data/backgrounds",
...
async function runApiFetcher() {
  ...
  try {
    ...
    if (TARGET_TYPE === "monsters") {
      ...
      while (hasMore) {
        ...
        const res = await axios.get(
          `${ENDPOINTS.monsters}?skip=${skip}&take=${take}`,
          { headers: reqHeaders },
        );
    ...
    // ROUTE 2: Spell Class-Sweep
      ...
      for (const classId of classIds) {
        try {
          ...
          const res = await axios.get(
            `${ENDPOINTS.spells}?classId=${classId}&classLevel=20`,
            { headers: reqHeaders },
          );
          ...
          /* Silently ignore deprecated classes */
        }
      ...
      for (const classId of classIds) {
        try {
          const res = await axios.get(
            `${ENDPOINTS.subclasses}?baseClassId=${classId}`,
            { headers: reqHeaders },
          );
          if (res.data && res.data.data) {
            res.data.data.forEach((subclass) => {
              ...
              if (
                actualSubclass.slug &&
                !uniqueSubclasses.has(actualSubclass.slug)
              ) {
    ...
    // ROUTE 3: Standard Fetch
      ...
      const res = await axios.get(ENDPOINTS[TARGET_TYPE], {
        headers: reqHeaders,
      });
    ...
    for (let rawItem of items) {
      ...
      if (TARGET_TYPE === "monsters") {
        const size =
          translateId(ddbConfig.monsterSizes, item.sizeId) || "Unknown Size";
        const type =
          translateId(ddbConfig.monsterTypes, item.typeId) || "Unknown Type";

        const stats = (item.stats || [])
          .map((s) => {
            const key =
              (ddbConfig.stats || []).find((st) => st.id === s.statId)?.key ||
              "STAT";
            return `**${key}:** ${s.value}`;
          })
          .join(" | ");
        ...
        const movements = (item.movements || [])
          .map((m) => {
            const label =
              translateId(ddbConfig.movements, m.movementId) || "Move";
        ...
        if (item.range?.rangeValue)
          rangeStr += ` (${item.range.rangeValue} ft.)`;
