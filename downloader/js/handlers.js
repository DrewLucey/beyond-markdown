...
export function processContent($, $content, sectionUrl, category = "GENERAL") {
  ...
  const $commentsHeader = $content
    .find("h1, h2, h3, h4, h5, h6, header")
    .filter((_, el) => {
  ...
  $content
    ...
    .filter((_, el) => {
      ...
      return (
        text.includes("when posting, please be sure") &&
  ...
  $content
    .find(
      '.ddb-action-icon--action, [data-original-title="Action"], [aria-label="Action"]',
    )
  ...
  $content.find(statBlockWrappers.join(", ")).each((_, el) => {
    ...
    if ($label.length > 0 && $value.length > 0) {
      ...
      if ($extra.length > 0)
        valueHtml += " " + ($extra.html() ? $extra.html().trim() : "");
  ...
  $content.find(blockquoteSelectors.join(", ")).each((_, el) => {
    ...
    if (
      $firstChild.length > 0 &&
      $firstChild[0].tagName.toLowerCase() === "p"
    ) {
  ...
  $content.find("a.lightbox").each((_, el) => {
    ...
    if (highResUrl)
      $a.replaceWith(`<img src="${highResUrl}" alt="${altText}">`);
  });

  // --- NESTED BOLD PREVENTER & LINK PRESERVER ---
  $content
    .find(
      ".tooltip-hover, .m-spell-hover, .monster-tooltip, .magic-item-tooltip, .rollable",
    )
    .each((_, el) => {
      const $el = $(el);
  ...
  $content.find(".ability-block, .stat-block-ability-scores").each((_, el) => {
    ...
    $attrBlock
      .find(".ability-block__stat, .stat-block-ability-scores-stat")
      .each((_, attr) => {
        const $attr = $(attr);
        const label = $attr
      ...
      $table.find("tr").each((_, tr) => {
        ...
        if ($cell.length > 0) {
          ...
          if (
            ...
            $cell.children("strong, b").length === 1
          ) {
  ...
  $content
    .find("div.hangingIndent, div.condensed-group.hangingIndent")
    .each((_, el) => {
      const $div = $(el);
